...
async def add_reseller(client, message):
    ...
    if len(parts) < 2: return await message.reply_text("`/add <user_id>`")
    target_id = parts[1].strip()
    if not target_id.isdigit(): return await message.reply_text("Please enter the User ID in numbers only.")
    if await db.add_reseller(target_id, f"User_{target_id}"):
        await message.reply_text(f"✅ Reseller ID `{target_id}` has been approved.")
    else:
...
async def remove_reseller(client, message):
    if message.from_user.id != OWNER_ID: return await message.reply_text("You are not the Owner.")
    parts = message.text.split()
    if len(parts) < 2: return await message.reply_text("Usage format - `/remove <user_id>`")
    target_id = parts[1].strip()
    if target_id == str(OWNER_ID): return await message.reply_text("The Owner cannot be removed.")
    if await db.remove_reseller(target_id):
        await message.reply_text(f"✅ Reseller ID `{target_id}` has been removed.")
    else:
...
async def set_cookie_command(client, message):
    ...
    if len(parts) < 2: return await message.reply_text("⚠️ **Usage format:**\n`/setcookie <Long_Main_Cookie>`")
...
async def handle_smart_cookie_update(client, message):
    ...
    try:
        ...
        if "PHPSESSID" not in extracted_cookies or "cf_clearance" not in extracted_cookies:
            return await message.reply_text("❌ <b>Error:</b> `PHPSESSID` နှင့် `cf_clearance` ကို ရှာမတွေ့ပါ။ Format မှန်ကန်ကြောင်း စစ်ဆေးပါ။")
...
async def add_balance_command(client, message):
    ...
    if len(parts) < 3: return await message.reply_text("⚠️ **Usage format:**\n`.addbal <User_ID> <Amount> [BR/PH]`")
    target_id = parts[1]
    try: amount = float(parts[2])
    except ValueError: return await message.reply_text("❌ Invalid amount.")
    currency = "BR"
    if len(parts) > 3:
...
async def handle_topup(client, message):
    ...
    loading_msg = await message.reply_text(f"Checking Code `{activation_code}`...")
...
async def execute_buy_process(message, lines, regex_pattern, currency, packages_dict, process_func, title_prefix, is_mcc=False):
    ...
    async with user_locks[tg_id]: 
        ...
        for line in lines:
            ...
            if not_found_pkgs:
                await message.reply_text(f"❌ Package(s) not found for ID {game_id}: {', '.join(not_found_pkgs)}")
                continue
...
async def handle_br_mlbb(client, message):
    if not await is_authorized(message.from_user.id): 
        return await message.reply_text(f"ɴᴏᴛ ᴀᴜᴛʜᴏʀɪᴢᴇᴅ ᴜsᴇʀ.❌")
    try:
        ...
        if total_pkgs > 10: 
            return await message.reply_text("❌ 10 Limit Exceeded: တစ်ကြိမ်လျှင် အများဆုံး ၁၀ ခုသာ ဝယ်ယူနိုင်ပါသည်။")
...
async def handle_br_mcc(client, message):
    ...
    try:
        ...
        if total_pkgs > 5: 
            return await message.reply_text("❌ 5 Limit Exceeded: တစ်ကြိမ်လျှင် အများဆုံး ၅ ခုသာ ဝယ်ယူနိုင်ပါသည်။")
...
async def check_official_customer(client, message):
    ...
    parts = message.text.strip().split()
    if len(parts) < 2:
        return await message.reply_text("⚠️ <b>Usage:</b> <code>.cus <Game_ID></code>")
        
    search_query = parts[1]
    loading_msg = await message.reply_text(f"Deep Searching Official Records for: <code>{search_query}</code>...")
    ...
    try:
        ...
        if not found_orders: 
            return await loading_msg.edit_text(f"❌ No successful records found for: <code>{search_query}</code>")
...
async def show_top_customers(client, message):
    ...
    if not top_spenders: return await message.reply_text("📜 No orders found in database.")
...
async def grant_vip_status(client, message):
    if message.from_user.id != OWNER_ID: return await message.reply_text("❌ Only Owner.")
    parts = message.text.strip().split()
    if len(parts) < 2: return await message.reply_text("⚠️ **Usage:** `.setvip <User_ID>`")
    target_id = parts[1]
...
async def toggle_maintenance(client, message):
    ...
    if len(parts) < 2 or parts[1] not in ["enable", "disable"]:
        return await message.reply_text("⚠️ **Usage:** `.maintenance enable` သို့မဟုတ် `.maintenance disable`")
...
async def add_scam_id(client, message):
    ...
    parts = message.text.strip().split()
    if len(parts) < 2:
        return await message.reply_text("⚠️ **Usage:** `.scam <Game_ID>`\nဥပမာ: `.scam 123456789`")
        
    scam_id = parts[1].strip()
    if not scam_id.isdigit():
        return await message.reply_text("❌ Invalid Game ID. ဂဏန်းများသာ ရိုက်ထည့်ပါ။")
...
async def remove_scam_id(client, message):
    ...
    if removed:
        await message.reply_text(f"✅ **Scammer ID Removed:** <code>{scam_id}</code>\nBlacklist ထဲမှ အောင်မြင်စွာ ဖယ်ရှားလိုက်ပါပြီ။")
    else:
...
async def show_scam_list(client, message):
    ...
    if not GLOBAL_SCAMMERS:
        return await message.reply_text("✅ ယခုလောလောဆယ် Blacklist သွင်းထားသော Scammer မရှိပါ။")
